        ...
        "D-Probe (SS)": {
          ...
          "D-Probe (GSSG)": {
            "65 GHz": [
                ...
                {"part_no": "DP-GSSG-601508", "pitch": 0.80},
                {"part_no": "DP-GSSG-601509", "pitch": 0.90},
            ]
        },
    }
}
...
MIL_TO_MM = 0.0254
if 'p_w_val' not in st.session_state: st.session_state.p_w_val = 0.0
if 'p_h_val' not in st.session_state: st.session_state.p_h_val = 0.0
if 'pad_pitch_val' not in st.session_state: st.session_state.pad_pitch_val = 0.0
if 'pad_layouts' not in st.session_state:
    st.session_state.pad_layouts = []
...
with st.expander("📌 Manage Multiple Target Pad Layouts", expanded=True):
    ...
    if st.button("➕ Add Current Layout"):
        if p_w_in <= 0 or p_h_in <= 0 or pitch_in <= 0:
            st.error("❌ Width, Height, and Pitch must all be greater than 0.")
        else:
...
if not st.session_state.pad_layouts:
    st.info("➕ Add at least one pad layout above to begin probe validation.")
else:
